...
def scrape_and_update(category, brand, model, year_from, year_to):
    ...
    # Tworzymy stałą sesję (Session), która utrzymuje połączenie jak prawdziwa przeglądarka
    ...
    # Rozbijamy wpisany model na części dla twardego filtru (np. "cla")
    model_parts = [re.sub(r'[^a-z0-9]', '', m) for m in model.lower().split()]
    
    page = 1
    max_pages = 25 # Zapas pozwalający na pobranie aż do 800 aut
    
    while page <= max_pages:
        # NOWY FORMAT LINKU: Używamy /od-{year_from}, co zapobiega złośliwemu przekierowywaniu przez Otomoto!
        url = f"https://www.otomoto.pl/{category_slug}/{brand_clean}/{model_clean}/od-{year_from}?search%5Bfilter_float_year%3Ato%5D={year_to}&page={page}"
        ...
        # Przerywamy TYLKO, gdy na stronie fizycznie nie ma już żadnych ofert (Otomoto mówi: koniec wyników)
        if not articles:
            break 
        ...
        for art in articles:
            try:
                ...
                # CENA - szukamy kwoty i bierzemy maksymalną, by pominąć raty leasingowe
                price = 0.0
                ...
                # ROCZNIK - omijamy lata jeśli cena została uznana za rocznik
                year_matches = re.findall(r'\b(19\d{2}|20\d{2})\b', raw_text)
                valid_year = None
                for ym in year_matches:
                    ym_int = int(ym)
                    # Upewniamy się, że to nie jest pojemność silnika ani część ceny
